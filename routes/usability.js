...
exports.alpha = function(req, res){
  var allEntries = Entry.find({}).sort('datecreated').exec(function (err, data) {
    if (err) {
      ...
      res.render('indexAlpha', {entries: []});
    } else {
      entry_compile(data, res, function (res, entrypack) {
        entrypack.reverse();
        res.render('indexAlpha', {entries: entrypack});
      });
    };
  });
};  
...
exports.beta = function(req, res){
  var allEntries = Entry.find({}).sort('datecreated').exec(function (err, data) {
    if (err) {
      ...
      res.render('indexBeta', {entries: []});
    } else {
      entry_compile(data, res, function (res, entrypack) {
        entrypack.reverse();
        res.render('indexBeta', {entries: entrypack});
      });
    };
  });
};  
...
// GAMMA
exports.gamma = function(req, res){
  var allEntries = Entry.find({}).sort('datecreated').exec(function (err, data) {
    if (err) {
      res.send("Could not find all entries");
    } else if (data.length == 0) {
      res.render('indexGamma', {entries: []});
    } else {
      entry_compile(data, res, function (res, entrypack) {
        entrypack.reverse();
        res.render('indexGamma', {entries: entrypack});
      });
    };
  });
};  
...
exports.addAlpha = function (req, res) {
  res.render('addAlpha');
};

exports.addBeta = function (req, res) {
  res.render('addBeta');
};

exports.addGamma = function (req, res) {
  res.render('addGamma');
};

exports.addAlpha_post = function(req, res) {
  ...
  newentry.save(function (err) {  
    if (err) {
      ...
      return res.redirect('/alpha');
    }
  });
};

exports.addBeta_post = function(req, res) {
  console.log(req.body.picture);
  var date = new Date();
  var newentry = new Entry({title: req.body.title, picture: req.body.picture, recipe: req.body.recipe,
                              category: req.body.category, notes: req.body.notes, datecreated: date});
...
exports.addGamma_post = function(req, res) {
  console.log(req.body.picture);
  var date = new Date();
  var newentry = new Entry({title: req.body.title, picture: req.body.picture, recipe: req.body.recipe,
                              category: req.body.category, notes: req.body.notes, datecreated: date});
